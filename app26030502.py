...
with tabs[3]:
    ...
    with col2:
        bl_no = st.text_input("B/L 번호 입력", placeholder="HBL 또는 MBL 번호를 입력하세요", key="bl_final_input")
    with col3:
        st.write("") 
    ...
    if search_btn:
        if not bl_no:
            ...
            with st.spinner("관세청 유니패스 서버 접속 중..."):
                ...
                try:
                    # 관세청 서버 지침: TLS 1.2 이상 권고 (requests 기본 지원)
                    ...
                    if response.status_code == 200:
                        ...
                        # [오류 메시지 체크] 가이드 공통 컬럼 ntceInfo 확인
                        ntce = root.findtext(".//ntceInfo")
                        if ntce and "[N00]" not in ntce and ntce.strip():
                            st.error(f"관세청 응답 메시지: {ntce}")
                            # 결과가 0건이면 여기서 중단
                            t_cnt = root.findtext(".//tCnt")
                            if not t_cnt or int(t_cnt) == 0: st.stop()
                        ...
                        if t_cnt and int(t_cnt) > 0:
                            ...
                            st.success(f"✅ 화물 상태: {info.findtext('csclPrgsStts')}")
                            
                            # 주요 요약 정보 (12px 스타일 적용)
                            m1, m2, m3 = st.columns(3)
                            with m1: st.metric("통관상태", info.findtext("csclPrgsStts"))
                            with m2: st.metric("입항세관", info.findtext("etprCstm"))
                            with m3: st.metric("총중량", f"{info.findtext('ttwg')} {info.findtext('wghtUt')}")

                            # 상세 요약 테이블
                            st.markdown("---")
                            summary_df = pd.DataFrame({
                                "항목": ["품명", "선박/항공기명", "입항일자", "현재위치", "MBL-HBL"],
                                "데이터": [
                                    info.findtext("prnm"),
                                    info.findtext("shipNm"),
                                    info.findtext("etprDt"),
                                    info.findtext("shcoFlcoSgn"),
                                    f"{info.findtext('mblNo')} / {info.findtext('hblNo')}"
                                ]
                            })
                            st.table(summary_df)

                            # 2. 진행 단계별 이력 (cargCsclPrgsInfoDtlQryVo)
                            st.markdown("#### 🕒 처리 단계별 상세 이력")
                            history = []
                            # 가이드에 따라 상세 내역 리스트 순회
                            for item in root.findall(".//cargCsclPrgsInfoDtlQryVo"):
                                history.append({
                                    "단계": item.findtext("cargTrcnRelaBsopTpcd"), # 처리구분
                                    "처리일시": item.findtext("prcsDttm"),
                                    "장치장/내용": item.findtext("shedNm") if item.findtext("shedNm") else item.findtext("rlbrCn"),
                                    "중량": f"{item.findtext('wght')} {item.findtext('wghtUt')}",
                            ...
                            st.warning("입력하신 정보로 조회된 화물이 없습니다. B/L 번호와 연도를 확인해 주세요.")
                            
                    elif response.status_code == 404:
                        st.error("❌ 404 에러: API 엔드포인트 경로를 찾을 수 없습니다. (서버 주소 설정 오류)")
                    else:
                        st.error(f"❌ 관세청 시스템 오류 (Status: {response.status_code})")
                        
                except Exception as e:
                    st.error(f"⚠️ 시스템 연결 실패: {str(e)}")
