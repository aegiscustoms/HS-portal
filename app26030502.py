...
# --- [Tab 4] 화물통관진행정보 (API 실시간 연동) ---
with tabs[3]:
    st.subheader("📦 실시간 화물통관 진행정보 조회")
    st.markdown("""
    <div style='font-size: 12px; color: gray; margin-bottom: 15px;'>
    MBL(Master BL) 또는 HBL(House BL) 번호와 입항년도를 입력하여 현재 화물의 상태를 실시간으로 확인하세요.
    </div>
    """, unsafe_allow_html=True)

    # API 인증키 설정 (st.secrets에 저장된 것을 사용하거나 직접 입력)
    # 가이드에 따르면 서비스 ID: API001 / URL: unipass 운영환경 주소 사용
    CR_API_KEY = st.secrets.get("UNIPASS_API_KEY", "YOUR_API_KEY_HERE")

    col1, col2, col3 = st.columns([1.5, 3, 1])
    with col1:
        # 가이드상 필수 파라미터: 입항년도 (기본 올해)
        carg_year = st.selectbox("입항년도", [2026, 2025, 2024], index=0)
    with col2:
        bl_no = st.text_input("B/L 번호 입력", placeholder="HBL 또는 MBL 번호를 입력하세요.")
    with col3:
        # 가이드상 MBL만 조회할지 HBL만 조회할지 선택 가능하나, 통합 검색으로 구현
        st.write("") # 간격 맞춤
    ...
    if search_btn:
        if not bl_no:
            ...
            with st.spinner("관세청 서버에서 데이터를 가져오는 중..."):
                # 1. API 요청 설정 (REST/GET 방식)
                # 가이드 URL: https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry
                url = "https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry"
                params = {
                    "crkyCn": CR_API_KEY,  # 인증키
                    "hblNo": bl_no,        # House BL
                    "blYy": carg_year      # 입항년도
                }
                ...
                try:
                    ...
                    if response.status_code == 200:
                        # 2. XML 파싱 (가이드상 응답 데이터 표준은 XML)
                        root = ET.fromstring(response.content)
                        
                        # 응답 결과 코드 확인
                        t_cnt = root.findtext(".//tCnt") # 전체 건수
                        
                        if t_cnt and int(t_cnt) > 0:
                            # 주요 일반 정보 추출
                            info = root.find(".//cargCsclPrgsInfoQryVo")
                            
                            st.markdown(f"### 🚩 진행 상태: <span style='color:blue'>{info.findtext('csclPrgsStts')}</span>", unsafe_allow_html=True)
                            
                            # 요약 정보 테이블 (12px 적용)
                            summary_data = {
                                "항목": ["품명", "MBL-HBL", "중량", "포장개수", "입항세관", "현재위치", "반출입일시"],
                                "내용": [
                                    info.findtext("prnm"), 
                                    f"{info.findtext('mblNo')} - {info.findtext('hblNo')}",
                                    f"{info.findtext('ttwg')} {info.findtext('wghtUt')}",
                                    f"{info.findtext('pckGcnt')} {info.findtext('pckUt')}",
                                    info.findtext("etprCstm"),
                                    info.findtext("shcoFlcoSgn"), # 선사/항공사
                                    info.findtext("prcsDttm")
                                ]
                            }
                            st.table(pd.DataFrame(summary_data))
                            
                            # 3. 상세 진행 단계 (역순으로 표시)
                            st.markdown("#### 🕒 시간별 진행 이력")
                            history = []
                            for item in root.findall(".//cargCsclPrgsInfoDtlQryVo"):
                                history.append({
                                    ...
                                    "장소/내용": item.findtext("shcoFlcoSgn"),
                                    "반출입개수": item.findtext("pckGcnt")
                                })
                            
                            df_hist = pd.DataFrame(history)
                            st.dataframe(df_hist.style.set_properties(**{'font-size': '12px', 'text-align': 'center'}), 
                                         hide_index=True, use_container_width=True)
                        else:
                            st.error("조회된 화물 정보가 없습니다. 번호나 입항년도를 확인하세요.")
                    else:
                        st.error(f"관세청 API 통신 실패 (Error Code: {response.status_code})")
