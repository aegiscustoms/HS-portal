...
# --- [Tab 4] 화물통관진행정보 (404 에러 완전 해결 버전) ---
with tabs[3]:
    st.subheader("📦 실시간 화물통관 진행정보 조회")
    
    # 1. Secrets에서 인증키 불러오기
    CR_API_KEY = st.secrets.get("UNIPASS_API_KEY", "").strip() # 공백 제거
    ...
    with col2:
        bl_no = st.text_input("B/L 번호 입력", placeholder="HBL 또는 MBL 번호", key="bl_real_input")
    with col3:
        st.write("") 
    ...
    if search_btn:
        if not bl_no:
            ...
            with st.spinner("관세청 유니패스 서버 연결 중..."):
                # [수정] 가이드 주소를 기반으로 가장 안정적인 URL 구조로 재설정
                # base_url과 endpoint를 분리하여 requests가 결합하도록 합니다.
                base_url = "https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry"
                
                # 파라미터 설정 (가이드 필수 항목: crkyCn, blYy, hblNo 또는 mblNo)
                # 여기서는 가장 일반적인 hblNo 조회를 기본으로 합니다.
                params = {
                    "crkyCn": CR_API_KEY,
                    "blYy": str(carg_year),
                    "hblNo": bl_no.strip().upper() # 대문자 변환 및 공백 제거
                }
                
                try:
                    # 헤더 추가 (필요한 경우 관세청 서버가 브라우저 요청을 차단하지 않도록 설정)
                    headers = {
                    ...
                    if response.status_code == 200:
                        ...
                        if t_cnt and int(t_cnt) > 0:
                            # 1. 요약 정보 (cargCsclPrgsInfoQryVo)
                            info = root.find(".//cargCsclPrgsInfoQryVo")
                            
                            st.success(f"✅ 화물 확인됨: {info.findtext('csclPrgsStts')}")
                            
                            # 메트릭 출력 (12px 스타일은 상단 CSS 적용분 활용)
                            ...
                            # 2. 상세 진행 이력 (cargCsclPrgsInfoDtlQryVo)
                            ...
                            for item in root.findall(".//cargCsclPrgsInfoDtlQryVo"):
                                history.append({
                                    ...
                                    "장소/선사": item.findtext("shcoFlcoSgn"),
                            ...
                            # 조회 결과는 0건이나 통신은 성공한 경우 (번호 오타 등)
                            ntce = root.findtext(".//ntceInfo")
                            st.warning(f"조회된 데이터가 없습니다. (메시지: {ntce if ntce else '번호 확인 필요'})")
                            
                    elif response.status_code == 404:
                        st.error("❌ [404 Error] 관세청 API 주소 설정 오류입니다. 주소 끝에 공백이 있거나 경로가 잘못되었습니다.")
                    else:
                        st.error(f"❌ 관세청 통신 에러 (Status: {response.status_code})")
