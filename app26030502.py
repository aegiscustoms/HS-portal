...
# --- [Tab 1] HS검색 (용도/재질 반영 및 출력 순서 최적화) ---
with tabs[0]:
    ...
    if st.button("HS분석 실행", use_container_width=True):
        if u_img or u_input:
            with st.spinner("전문 관세사 AI가 분석 중..."):
                try:
                    # 1) 및 2) 내용 반영 프롬프트
                    prompt = f"""당신은 전문 관세사입니다. 아래 지침에 따라 HS코드를 분류하세요.
                    
                    [입력 정보 분석]
                    - 사용자 입력값('{u_input}')에는 물품의 용도, 기능, 성분, 재질 정보가 포함되어 있을 수 있습니다. 
                    - 이미지와 입력된 텍스트 정보를 결합하여 최적의 10자리 HSK 코드를 도출하세요.

                    [출력 형식 및 순서]
                    ㄱ) 품명: (입력값이 있으면 우선 사용, 없으면 이미지 기반 예상 품명 제시)
                    ㄴ) 추천결과:
                       - 1순위가 100% 확정인 경우: 1순위 [코드] 100% 만 출력하고 종료.
                       - 미확정인 경우: 6단위 또는 10단위 기준 상위 3순위까지 추천하되, 하위 순위의 확률이 매우 낮아 의미가 없는 경우(예: 합계가 100%에 도달하거나 3순위가 0%에 가까운 경우)에는 1순위와 2순위만 출력.
                       - 각 순위는 'n순위 [코드] [확률]%' 형식으로 한 줄씩 출력.
                    """
                    ...
                    # 100% 확정 시 하단에 DB 상세 정보 연동 (기존 로직 유지)
