        ...
        if target_hs:
            ...
            if not m.empty:
                ...
                if not r.empty:
                    ...
                    m1, m2 = st.columns(2)
                    # metric 출력 시 f-string을 사용하여 numpy 타입 에러 방지
                ...
                st.error("해당 HS코드의 마스터 정보가 DB에 존재하지 않습니다. [관리자] 탭에서 최신 CSV를 업로드해 주세요.")
        else:
            st.warning("HS코드를 입력해 주세요.")
