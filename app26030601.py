...
# --- [Tab 5] 세액계산기 (HS코드 연동 및 요청 사항 반영) ---
with tabs[4]:
    ...
    with st.expander("📍 1. 과세가격(CIF) 및 품목 입력", expanded=True):
        ...
        with col_hs:
            ...
            # HS코드 입력 시 DB에서 관세율 가져오기
            db_duty_rate = 8.0  # 기본값
            if input_hs:
                hsk_clean = re.sub(r'[^0-9]', '', input_hs).zfill(10)
                conn = sqlite3.connect("customs_master.db")
                # rates 테이블에서 해당 HS코드의 관세율 조회 (가장 일반적인 A세율 우선 시도)
                rate_df = pd.read_sql(f"SELECT rate FROM rates WHERE hs_code = '{hsk_clean}' ORDER BY type ASC LIMIT 1", conn)
                conn.close()
                if not rate_df.empty:
                    try:
                        db_duty_rate = float(rate_df['rate'].values[0])
                        st.caption(f"✅ DB 연동: HS {hsk_clean}의 관세율({db_duty_rate}%)을 불러왔습니다.")
                    except: pass
    ...
    if st.button("세액 계산 실행", use_container_width=True):
        ...
        # 4. 교육세 (과표: 개소세액 또는 주세액)
        tax_edu = int((tax_excise + tax_liquor) * (edu_rate / 100))
        
        # 5. 농어촌특별세
        tax_rural = int((tax_duty + tax_excise) * (rural_rate / 100)) # 실무에선 감면분 기준이나 입력값 기준으로 간소화
        
        # 6. 부가가치세 (과표: CIF + 관세 + 개소세 + 주세 + 교육세 + 농특세)
        ...
        # 결과 리포트 (요청 순서: 관세, 부가세, 개소세, 주세, 교육세, 농특세)
