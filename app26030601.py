...
# --- [Tab 4] 화물통관진행정보 (실시간 화물 추적) ---
with tabs[3]:
    st.markdown(f"<div style='font-size: {TITLE_FONT_SIZE}; font-weight: bold; color: #1E3A8A;'>📦 실시간 화물 추적 (Cargo Tracking)</div>", unsafe_allow_html=True)
    CARGO_KEY = st.secrets.get("CARGO_API_KEY", "").strip()
    
    col_y, col_b = st.columns([1, 2])
    with col_y: yy = st.selectbox("입항년도", [2026, 2025, 2024], index=0)
    with col_b: bl_no = st.text_input("B/L 번호 (HBL/MBL)", placeholder="번호 입력")

    if st.button("화물 실시간 추적", use_container_width=True):
        if not bl_no: st.warning("B/L 번호를 입력하세요.")
        else:
            with st.spinner("화물 이동 경로 추적 중..."):
                url = "https://unipass.customs.go.kr:38010/ext/rest/cargCsclPrgsInfoQry/retrieveCargCsclPrgsInfo"
                params = {"crkyCn": CARGO_KEY, "blYy": str(yy), "hblNo": bl_no.strip().upper()}
                try:
                    res = requests.get(url, params=params, headers=HTTP_HEADERS, timeout=15)
                    root = ET.fromstring(res.content)
                    ...
                    if info is not None:
                        ...
                        st.info(f"품명: {info.findtext('prnm')} | 중량: {info.findtext('ttwg')} {info.findtext('wghtUt')}")
                    else: st.warning("조회된 화물 정보가 없습니다.")
                except Exception as e: st.error(f"API 호출 실패: {e}")
