...
st.markdown(f"""
    <style>
        ...
        * {{ font-family: 'Pretendard', sans-serif; }}
        ...
        /* 중앙 정렬 테이블 전역 설정 */
        .center-table {{ width: 100%; text-align: center !important; border-collapse: collapse; margin-top: 10px; }}
        .center-table th {{ background-color: #F8FAFC !important; color: #1E3A8A !important; text-align: center !important; padding: 10px !important; border-bottom: 2px solid #E2E8F0; font-size: {CONTENT_FONT_SIZE}; }}
        .center-table td {{ text-align: center !important; padding: 8px !important; border-bottom: 1px solid #F1F5F9; font-size: {CONTENT_FONT_SIZE}; }}
    </style>
...
def init_db():
    ...
    ca = conn_auth.cursor() # 커서 명시적 생성 #변경내역★★
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            try:
                ...
                # 2) 관세율 (rate_names와 조인하여 명칭 확보) #변경내역★★
                r_query = f"""
                ...
                # 3) 요건 (필드명 PREUI 사양으로 강제 지정) #변경내역★★
                ...
                if not m.empty:
                    ...
                    st.divider()
                    st.markdown("<div class='custom-header'>💰 관세율 정보</div>", unsafe_allow_html=True) #변경내역★★
                    if not r_all.empty:
                        ...
                        st.write(re_etc.to_html(index=False, classes='center-table'), unsafe_allow_html=True)
                        
                        st.markdown("**협정세율 (FTA)**") #변경내역★★
                        st.write(rf.to_html(index=False, classes='center-table'), unsafe_allow_html=True)
                    
                    st.divider()
                    st.markdown("<div class='custom-header'>🛡️ 세관장확인대상 (수출입요건)</div>", unsafe_allow_html=True) #변경내역★★
                    ci, ce = st.columns(2)
                    with ci: 
                        st.markdown("**[수입 요건]**") #변경내역★★
                        st.write(req_i.to_html(index=False, classes='center-table'), unsafe_allow_html=True)
                    with ce: 
                        st.markdown("**[수출 요건]**") #변경내역★★
                        st.write(req_e.to_html(index=False, classes='center-table'), unsafe_allow_html=True)
                else: st.warning("해당 HS코드의 데이터를 찾을 수 없습니다. 탭6에서 마스터 데이터를 먼저 업로드해 주세요.")
