...
# --- [Tab 2] HS정보 (DatabaseError 방지 로직 강화) ---
with tabs[1]:
    ...
    if st.button("데이터 통합 조회", use_container_width=True):
        if target_hs:
            ...
            try:
                ...
                else: st.warning("HS코드를 찾을 수 없습니다. 탭6에서 마스터 데이터를 먼저 업로드하세요.")
            except Exception as e: st.error(f"DB 조회 오류: {e}\n(관리자 페이지에서 데이터를 다시 '반영'해 보세요.)")
            ...
            with cols[i%3]:
                ...
                if up and st.button("반영", key=f"btn_{m_name}"):
                    ...
                    if df is not None:
                        try:
                            conn = sqlite3.connect("customs_master.db")
                            # ★ [강제 컬럼 매핑 로직] 파일의 헤더가 무엇이든 시스템 표준으로 변경
                            if m_name == "HS코드(마스터)":
                                df = df.iloc[:, [0, 1, 2]] # 순서대로 3개 열 추출
                                df.columns = ['hs_code', 'name_kr', 'name_en']
                            elif m_name == "표준품명":
                                df = df.iloc[:, [1, 0, 2, 3]] # 순서 기반 매핑
                                df.columns = ['hs_code', 'base_name', 'std_name_kr', 'std_name_en']
                            elif m_name == "관세율":
                                df = df.iloc[:, [0, 1, 2]]
                                df.columns = ['hs_code', 'type', 'rate']
                            elif m_name == "세관장확인(수입)" or m_name == "세관장확인(수출)":
                                df = df.iloc[:, [0, 1, 2, 3]]
                            ...
                            if m_name in t_map: df.to_sql(t_map[m_name], conn, if_exists='replace', index=False)
                                ...
                                df.columns = ['code', 'h_name']
                                df.to_sql("rate_names", conn, if_exists='replace', index=False)
                ...
                if s_up and st.button("반영", key=f"sbtn_{s_name}"):
                    ...
                    if sdf is not None:
                        try:
                            ...
                            # 통계부호도 순서 기반 강제 매핑
                            if s_name == "간이세율(2026)":
                                sdf = sdf.iloc[:, [0, 1, 2]]; sdf.columns = ['gani_hs', 'gani_name', 'rate']
                            elif s_name == "관세감면부호(2026)":
                                sdf = sdf.iloc[:, [0, 1, 2, 3, 4, 5]]; sdf.columns = ['code', 'content', 'rate', 'after_target', 'installment_months', 'installment_count']
                            elif s_name == "내국세면세부호(2026)":
                                sdf = sdf.iloc[:, [0, 1, 2]]; sdf.columns = ['name', 'type_name', 'code']
                            elif s_name == "내국세율(2026)":
                                sdf = sdf.iloc[:, [0, 1, 2, 3, 4, 5, 6, 7]]; sdf.columns = ['item_name', 'tax_rate', 'type_code', 'type_name', 'tax_kind_code', 'unit', 'tax_base_price', 'agri_tax_yn']
