...
with tabs[3]:
            ...
            with st.spinner("관세청 유니패스 통신 중..."):
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if info is not None:
                            # [핵심] 결과값을 세션에 저장하여 리런 후에도 유지되게 함 #변경내역★★
                            ...
                            # 값이 바뀌었을 때만 리런하여 무한 루프 및 증발 방지 #변경내역★★
    ...
    # 2. 결과 출력 영역 (세션에 저장된 결과를 화면에 그림) #변경내역★★
    if st.session_state.search_result:
        ...
        if info is not None:
            def get_safe_text(element, tag, length=None):
                ...
                return (val[:length] if length else val) if val else "정보없음"
            ...
            if history:
                ...
                # 탭2에서 설정한 full-width-table 스타일 적용
